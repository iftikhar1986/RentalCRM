...
async function createDynamicLeadSchema(fieldConfigs: any[]) {
  // Base schema with all required database fields
  const baseSchema = {
  ...
  // Apply field configurations to modify requirements
  const schemaFields: Record<string, any> = { ...baseSchema };
...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.post("/api/leads", isAuthenticated, async (req, res) => {
    try {
      ...
      // Use the basic insertLeadSchema for validation
      ...
      console.log("Creating lead with processed data:", JSON.stringify(leadDataWithDefaults, null, 2));
  ...
  app.patch("/api/leads/:id", isAuthenticated, async (req, res) => {
    try {
      ...
      console.log("Received lead update request for ID:", id);
      console.log("Update data:", JSON.stringify(req.body, null, 2));
      
      const parsed = updateLeadSchema.safeParse(req.body);
      
      if (!parsed.success) {
        console.log("Update validation errors:", parsed.error.errors);
        const validationError = fromZodError(parsed.error);
        return res.status(400).json({ message: validationError.message });
      }
      ...
      console.log("Updating lead with parsed data:", JSON.stringify(parsed.data, null, 2));
  ...
  app.post("/api/users", requireAdmin, async (req, res) => {
    try {
      ...
      const result = extendedSchema.safeParse(req.body);
      if (!result.success) {
      ...
      const user = await storage.createStaffUser(result.data);
  ...
  app.post("/api/branches/:branchId/users", requireAdmin, async (req, res) => {
    try {
      ...
      if (!parsed.success) {
        ...
        const userFriendlyMessage = fieldErrors.length === 1 
          ? fieldErrors[0].message
          : `Please check the following: ${fieldErrors.map(e => e.message.toLowerCase()).join(', ')}`;
      ...
      res.status(500).json({ message: "Unable to create staff member. Please try again." });
    }
  });
  ...
  // Vehicle Types - GET all for admin management (active and inactive) - MUST come before general route
  app.get("/api/vehicles/types/all", requireAdmin, async (req, res) => {
    try {
      const vehicleTypes = await storage.getAllVehicleTypes();
      res.json(vehicleTypes);
    } catch (error) {
      console.error("Error fetching all vehicle types:", error);
      res.status(500).json({ message: "Failed to fetch all vehicle types" });
    }
  });

  // Vehicle Types - GET available to all authenticated users for lead form (active only)
  ...
  app.post("/api/vehicles/types", requireAdmin, async (req, res) => {
    try {
  ...
  app.put("/api/vehicles/types/:id", requireAdmin, async (req, res) => {
    try {
  ...
  app.delete("/api/vehicles/types/:id", requireAdmin, async (req, res) => {
    try {
  ...
  // Vehicle Makes - GET all for admin management (active and inactive) - MUST come before general route
  app.get("/api/vehicles/makes/all", requireAdmin, async (req, res) => {
    try {
      const vehicleMakes = await storage.getAllVehicleMakes();
      res.json(vehicleMakes);
    } catch (error) {
      console.error("Error fetching all vehicle makes:", error);
      res.status(500).json({ message: "Failed to fetch all vehicle makes" });
    }
  });

  // Vehicle Makes - GET available to all authenticated users for lead form (active only)
  ...
  // Get makes by type
  app.get("/api/vehicles/makes/by-type/:typeId", isAuthenticated, async (req, res) => {
    try {
      const vehicleMakes = await storage.getVehicleMakesByType(req.params.typeId);
      res.json(vehicleMakes);
    } catch (error) {
      console.error("Error fetching vehicle makes by type:", error);
      res.status(500).json({ message: "Failed to fetch vehicle makes by type" });
    }
  });

  app.post("/api/vehicles/makes", requireAdmin, async (req, res) => {
    try {
  ...
  app.put("/api/vehicles/makes/:id", requireAdmin, async (req, res) => {
    try {
  ...
  app.delete("/api/vehicles/makes/:id", requireAdmin, async (req, res) => {
    try {
  ...
  // Vehicle Models - GET all for admin management (active and inactive) - MUST come before general route
  app.get("/api/vehicles/models/all", requireAdmin, async (req, res) => {
    try {
      const vehicleModels = await storage.getAllVehicleModels();
      res.json(vehicleModels);
    } catch (error) {
      console.error("Error fetching all vehicle models:", error);
      res.status(500).json({ message: "Failed to fetch all vehicle models" });
    }
  });

  // Vehicle Models - GET available to all authenticated users for lead form (active only)
  ...
  // Get models by make
  app.get("/api/vehicles/models/by-make/:makeId", isAuthenticated, async (req, res) => {
    try {
      const vehicleModels = await storage.getVehicleModelsByMake(req.params.makeId);
      res.json(vehicleModels);
    } catch (error) {
      console.error("Error fetching vehicle models by make:", error);
      res.status(500).json({ message: "Failed to fetch vehicle models by make" });
    }
  });

  app.post("/api/vehicles/models", requireAdmin, async (req, res) => {
    try {
  ...
  app.put("/api/vehicles/models/:id", requireAdmin, async (req, res) => {
    try {
  ...
  app.delete("/api/vehicles/models/:id", requireAdmin, async (req, res) => {
    try {
  ...
  // Vehicle Plates - GET all for admin management (active and inactive) - MUST come before general route
  app.get("/api/vehicles/plates/all", requireAdmin, async (req, res) => {
    try {
      const vehiclePlates = await storage.getAllVehiclePlates();
      res.json(vehiclePlates);
    } catch (error) {
      console.error("Error fetching all vehicle plates:", error);
      res.status(500).json({ message: "Failed to fetch all vehicle plates" });
    }
  });

  // Vehicle Plates - GET active only
  ...
  app.post("/api/vehicles/plates", requireAdmin, async (req, res) => {
    try {
      ...
      const existingPlate = await storage.getVehiclePlates();
      const plateExists = existingPlate.find(plate => 
        plate.plateNumber === result.data.plateNumber
      );
      
      if (plateExists) {
        return res.status(400).json({ 
          message: `Plate number "${result.data.plateNumber}" is already registered in the system. Please choose a unique plate number for this vehicle.`
        });
      }
  ...
  app.put("/api/vehicles/plates/:id", requireAdmin, async (req, res) => {
    try {
  ...
  app.delete("/api/vehicles/plates/:id", requireAdmin, async (req, res) => {
    try {
    ...
    try {
        ...
        typesSheet.eachRow((row, rowNumber) => {
          ...
          if (name && String(name).trim()) {
            vehicleData.vehicleTypes.push({ 
              ...
              isActive: true // Ensure new types are active by default
            });
          }
        });
      }

      // Store raw makes data first (needs type lookup)
      const rawMakes: any[] = [];
      ...
      if (makesSheet) {
        makesSheet.eachRow((row, rowNumber) => {
          ...
          const values = row.values as any[];
          const name = values[1];
          const typeName = values[2];
          if (name && String(name).trim() && typeName) {
            rawMakes.push({ 
              name: String(name).trim(), 
      ...
      // Process Models sheet (store raw data first)
      ...
      if (modelsSheet) {
        modelsSheet.eachRow((row, rowNumber) => {
          ...
          if (name && String(name).trim() && makeName) {
            rawModels.push({
              name: String(name).trim(),
      ...
      // Process Plates sheet (store raw data first)
      ...
      // First, upload types only
      let firstUploadResult;
      if (vehicleData.vehicleTypes.length > 0) {
        firstUploadResult = await storage.bulkUploadVehicles({
      ...
      // Now get all types (including newly created ones) and process makes
      const allTypes = await storage.getVehicleTypes();
      
      // Process makes with type lookup
      for (const rawMake of rawMakes) {
        const type = allTypes.find(t => t.name.toLowerCase() === rawMake.typeName.toLowerCase() && t.isActive);
        ...
        if (type) {
          ...
          console.log(`Skipping make ${rawMake.name}: Type "${rawMake.typeName}" not found or inactive`);
        }
      }

      // Upload makes
      let secondUploadResult;
      if (vehicleData.vehicleMakes.length > 0) {
        secondUploadResult = await storage.bulkUploadVehicles({
          vehicleTypes: [],
          vehicleMakes: vehicleData.vehicleMakes,
          vehicleModels: [],
          vehiclePlates: []
        });
      }

      // Now get all makes including newly created ones
      const allMakes = await storage.getVehicleMakes();

      // Process models with make lookup only (type is inherited from make)
      for (const rawModel of rawModels) {
        const make = allMakes.find(m => m.name.toLowerCase() === rawModel.makeName.toLowerCase() && m.isActive);
        ...
        if (make) {
          ...
          console.log(`Skipping model ${rawModel.name}: Make "${rawModel.makeName}" not found or inactive`);
        }
      }

      // Upload models
      let thirdUploadResult;
      if (vehicleData.vehicleModels.length > 0) {
        thirdUploadResult = await storage.bulkUploadVehicles({
          vehicleTypes: [],
          vehicleMakes: [],
      ...
      // Get all models including newly created ones
      const allModels = await storage.getVehicleModels();

      // Process plates with updated lookup data
      for (const rawPlate of rawPlates) {
        const model = allModels.find(m => m.name.toLowerCase() === rawPlate.modelName.toLowerCase() && m.isActive);
        
        if (model) {
          vehicleData.vehiclePlates.push({
            plateNumber: rawPlate.plateNumber,
            modelId: model.id,
            color: rawPlate.color,
            status: rawPlate.status
          });
        } else {
          console.log(`Skipping plate ${rawPlate.plateNumber}: Model "${rawPlate.modelName}" not found or inactive`);
        }
      }

      // Upload plates
      let fourthUploadResult;
      if (vehicleData.vehiclePlates.length > 0) {
        fourthUploadResult = await storage.bulkUploadVehicles({
          vehicleTypes: [],
          vehicleMakes: [],
          vehicleModels: [],
          vehiclePlates: vehicleData.vehiclePlates
        });
      }

      // Combine all results
      const uploadResult = {
        vehicleTypes: firstUploadResult?.vehicleTypes || [],
        vehicleMakes: secondUploadResult?.vehicleMakes || [],
        vehicleModels: thirdUploadResult?.vehicleModels || [],
        vehiclePlates: fourthUploadResult?.vehiclePlates || [],
        summary: {
          typesCreated: firstUploadResult?.summary?.typesCreated || 0,
          makesCreated: secondUploadResult?.summary?.makesCreated || 0,
          modelsCreated: thirdUploadResult?.summary?.modelsCreated || 0,
          platesCreated: fourthUploadResult?.summary?.platesCreated || 0,
        }
      };
      console.log('Bulk upload completed successfully:', uploadResult);
      res.status(201).json(uploadResult);
    } catch (error) {
      console.error("Error in Excel bulk upload:", error);
      res.status(500).json({ message: "Failed to process Excel file. Please check the file format and try again." });
    }
  });
  ...
  app.get("/api/vehicles/bulk-upload-template", isAuthenticated, requireAdmin, async (req, res) => {
    try {
      ...
      // Create Makes sheet (with Type column)
      const makesSheet = workbook.addWorksheet('Makes');
      makesSheet.addRow(['Name', 'Type']);
      makesSheet.addRow(['Toyota', 'SUV']);
      makesSheet.addRow(['Honda', 'Sedan']);
      makesSheet.addRow(['Nissan', 'Hatchback']);

      // Create Models sheet (Type is inherited from Make)
      const modelsSheet = workbook.addWorksheet('Models');
      modelsSheet.addRow(['Name', 'Make Name', 'Year']);
      modelsSheet.addRow(['Camry', 'Toyota', 2024]);
      modelsSheet.addRow(['Prado', 'Toyota', 2024]);
      modelsSheet.addRow(['Civic', 'Honda', 2024]);
